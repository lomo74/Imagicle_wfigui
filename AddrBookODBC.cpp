...
void __fastcall TAddressBookODBC::Load()
{
	...
	try {
		...
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to initialize ODBC: ") +
			GetSQLMessages(SQL_HANDLE_ENV, SQL_NULL_HANDLE));

		rc = SQLSetEnvAttr(hEnv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to setup ODBC: ") +
			GetSQLMessages(SQL_HANDLE_ENV, hEnv));

		//hCon
		rc = SQLAllocHandle(SQL_HANDLE_DBC, hEnv, &hCon);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to create ODBC connection: ") +
			GetSQLMessages(SQL_HANDLE_ENV, hEnv));

		rc = SQLSetConnectAttrW(hCon, SQL_LOGIN_TIMEOUT, (SQLPOINTER)10, 0);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to setup ODBC connection: ") +
			GetSQLMessages(SQL_HANDLE_ENV, hEnv));
		...
		//connect
		if (Config->ODBCAuth)
			rc = SQLConnectW(hCon, Config->ODBCDSN.c_str(), SQL_NTS,
			Config->ODBCUid.c_str(), SQL_NTS,
			Config->ODBCPwd.c_str(), SQL_NTS);
		else
			rc = SQLConnectW(hCon, Config->ODBCDSN.c_str(), SQL_NTS,
			NULL, 0, NULL, 0);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to open ODBC connection: ") +
			GetSQLMessages(SQL_HANDLE_DBC, hCon));

		//get identifier quote char
		SQLWCHAR buf[8];
		const SQLWCHAR *szQuoteChar;
		SQLSMALLINT len;
		rc = SQLGetInfoW(hCon, SQL_IDENTIFIER_QUOTE_CHAR, buf, (SQLSMALLINT)sizeof(buf), &len);
		if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
			szQuoteChar = buf;
		else
			szQuoteChar = static_cast<const SQLWCHAR *>(L"\0");

		//hStmt
		rc = SQLAllocHandle(SQL_HANDLE_STMT, hCon, &hStmt);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to create ODBC statement: ") +
			GetSQLMessages(SQL_HANDLE_DBC, hCon));

		UnicodeString stmt, name, fax, table;
		//quote all identifiers
		table.sprintf(L"%s%s%s", szQuoteChar, Config->ODBCTable.c_str(), szQuoteChar);
		name.sprintf(L"%s%s%s", szQuoteChar, Config->ODBCNameField.c_str(), szQuoteChar);
		fax.sprintf(L"%s%s%s", szQuoteChar, Config->ODBCFaxField.c_str(), szQuoteChar);

		//build SQL statement and execute it
		stmt.sprintf(szSelectTmpl,
			name.c_str(), fax.c_str(),		//SELECT
			table.c_str(),					//FROM
			name.c_str(), name.c_str(),		//WHERE
			fax.c_str(), fax.c_str(),		//AND
			name.c_str()					//ORDER BY
		);
		rc = SQLExecDirectW(hStmt, (SQLWCHAR *)stmt.c_str(), SQL_NTS);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to execute ODBC statement: ") +
			GetSQLMessages(SQL_HANDLE_STMT, hStmt));

		//bind columns
		rc = SQLBindCol(hStmt, 1, SQL_C_WCHAR, szName, NAME_LEN, &cbName);
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
			throw EAddressBookException(_(L"Unable to bind ODBC column: ") +
			GetSQLMessages(SQL_HANDLE_STMT, hStmt));
		...
		do {
			rc = SQLFetch(hStmt);
			if (rc == SQL_ERROR) {
				throw EAddressBookException(_(L"Unable to fetch ODBC data: ") +
				GetSQLMessages(SQL_HANDLE_STMT, hStmt));
