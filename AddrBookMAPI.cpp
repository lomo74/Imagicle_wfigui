			...
			while (!bEOF) {
				...
				try {
					if (FAILED(hr = pCont->QueryRows(50, 0, &pRows)))
						throw EAddressBookException(_(L"Unable to query address book contents table: 0x") + IntToHex((int)hr, 8));
			...
			while (!bEOF) {
				...
				try {
					if (FAILED(hr = pHier->QueryRows(100, 0, &pRows)))
						throw EAddressBookException(_(L"Unable to query address book hierarchy table: 0x") + IntToHex((int)hr, 8));
					...
					for (ULONG row = 0; row < pRows->cRows; row++) {
						...
						(LPUNKNOWN *)&pSubCont)))
							throw EAddressBookException(_(L"Unable to open address book sub container: 0x") + IntToHex((int)hr, 8));
...
void __fastcall TAddressBookMAPI::Load()
{
	...
	try {
		...
		if (FAILED(hrMapi = ::MAPIInitialize(NULL)))
			throw EAddressBookException(_(L"Unable to initialize MAPI: 0x") + IntToHex((int)hrMapi, 8));
		...
		ULONG flags = MAPI_NEW_SESSION |
  					...
  					MAPI_NO_MAIL |
  					MAPI_UNICODE;
		if (Config->MAPIUseDefProfile)
			flags |= MAPI_USE_DEFAULT;
		else
			profile = Config->MAPIProfile.c_str();
		...
		if (FAILED(hr))
			throw EAddressBookException(_(L"Unable to logon to MAPI: 0x") + IntToHex((int)hr, 8));

		try {
			//open address book
			if (FAILED(hr = pSess->OpenAddressBook(0, NULL, 0, &pAddrBook)))
				throw EAddressBookException(_(L"Unable to open MAPI address book: 0x") + IntToHex((int)hr, 8));

			//open container
			ULONG obj_type;
			if (FAILED(hr = pAddrBook->OpenEntry(0, NULL, NULL, 0, &obj_type, (LPUNKNOWN *)&pRootCont)))
				throw EAddressBookException(_(L"Unable to open MAPI address book container: 0x") + IntToHex((int)hr, 8));
...
#ifdef MAPIDEBUG
			if (MAPImsg.Length() > 0) {
				TFileStream *log = new TFileStream(Config->WFIUserDir + L"wfimapi.log", fmCreate);
				try {
					WORD BOM = 0xFEFF;
